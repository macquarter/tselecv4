...
const staticNews: NewsItem[] = [
  ...
  { id: 'n2', cat: '보도자료', title: '태승전자, 스마트팩토리 고도화 프로젝트 성공적 완료', content: '태승전자(주)는 인천 로봇랜드 공장의 스마트팩토리 고도화 프로젝트를 성공적으로 완료했습니다.\n\n이번 프로젝트를 통해 생산 효율이 35% 향상되었으며, 불량률이 0.1% 이하로 감소했습니다.', date: '2026.02.28', views: 287, author: '관리자' },
  { id: 'n3', cat: '공지사항', title: 'ISO 14001 환경경영시스템 인증 갱신', content: '태승전자(주)는 ISO 14001:2015 환경경영시스템 인증을 성공적으로 갱신하였습니다.', date: '2026.01.10', views: 156, author: '관리자' },
  { id: 'n4', cat: '보도자료', title: '2025 한국전자전 참가 안내', content: '태승전자(주)는 2025 한국전자전(KES 2025)에 참가합니다.\n\n전시 기간: 2025.10.21 ~ 10.24\n장소: 코엑스 Hall A', date: '2025.10.15', views: 198, author: '관리자' },
  { id: 'n5', cat: '공지사항', title: '2025년 추석 연휴 휴무 안내', content: '2025년 추석 연휴 휴무 안내드립니다.\n\n휴무 기간: 2025.10.03(금) ~ 10.07(화)\n업무 재개: 2025.10.08(수)', date: '2025.09.25', views: 89, author: '관리자' },
  { id: 'n6', cat: '이벤트', title: '태승전자 창립 36주년 기념 이벤트', content: '태승전자 창립 36주년을 맞아 기념 이벤트를 진행합니다.', date: '2025.07.01', views: 234, author: '관리자' },
];
              ...
              <motion.div key="list" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
                <div className="flex flex-col sm:flex-row gap-3 mb-6">
                  <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="검색어 입력..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-gray-500 outline-none focus:border-white/20 transition-colors" />
                  </div>
                </div>
                <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
                  {categories.map(cat => (
                    <button key={cat} onClick={() => setFilter(cat)} className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap border transition-all ${filter === cat ? 'bg-white/10 border-white/15 text-white' : 'bg-white/[0.02] border-white/5 text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}>{cat}</button>
                  ))}
                </div>
                <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden">
                  {filtered.length === 0 ? (
                    <div className="py-16 text-center text-gray-500 text-sm">{search ? '검색 결과가 없습니다' : '등록된 글이 없습니다'}</div>
                  ) : (
